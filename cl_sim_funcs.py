...
def plot_3d_fields(mesh, colour, ax=None, cmap=cm.seismic, col_scale=(-1,1)):
    ...
    maxcol = np.max(np.abs(colour))
    if col_scale==[-1, 1]:
        data_plus = (colour/maxcol + 1)/2
        col_min = -np.max(np.abs(colour))
        col_max = np.max(np.abs(colour))
    else:
        data_plus = colour/maxcol
        col_min = 0
        col_max = np.max(np.abs(colour))
...
#    ax.auto_scale_xyz(scale, scale, scale)
#    
#    maxcol = np.max(np.abs(colour1))
#    if col_scale==[-1, 1]:
#        data_plus = (colour1/maxcol + 1)/2
#        col_min = -np.max(np.abs(colour1))
#        col_max = np.max(np.abs(colour1))
#    else:
#        data_plus = colour1/maxcol
#        col_min = 0
#        col_max = np.max(np.abs(colour1))
##    poly_sph.set_facecolors(cmap(data_plus))
#    
#    # Set facecolours
